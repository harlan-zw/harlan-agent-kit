...
<template>
  <!-- NEW badge mode -->
  <span v-if="isNew" data-ui="UiTrend" class="status-success inline-flex items-center rounded-sm bg-success/10 px-1.5 py-0.5 font-medium uppercase" :class="sizes[size]">
    New
  </span>
  <!-- LOST badge mode (explicit or auto-detected from -100% change) -->
  <span v-else-if="isLost || autoLost" data-ui="UiTrend" class="status-error inline-flex items-center rounded-sm bg-error/10 px-1.5 py-0.5 font-medium uppercase" :class="sizes[size]">
    Lost
  </span>
  <!-- Zero trend: a dash, not a zero, so the eye does not read a value -->
  <span v-else-if="trend === 0" data-ui="UiTrend" class="text-dimmed" :class="sizes[size]" role="img" aria-label="No change">–</span>
