...
import type { UiStatProps } from './Stat.vue'
// Port of nuxtseo design-system UiStats. Plain filter+join instead of
// tailwind-merge — the component generates no conflicting Tailwind classes,
...
const containerClass = computed(() => {
  ...
  return join(
    ...
    wrap && 'flex-wrap',
    variant === 'card' && 'divide-y md:divide-y-0 md:divide-x divide-default rounded-lg border border-default py-4',
    variant === 'cards' && 'gap-4',
  )
})
