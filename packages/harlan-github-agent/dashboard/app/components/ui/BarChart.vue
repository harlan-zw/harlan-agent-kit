...
// Port of nuxtseo design-system UiBarChart. Horizontal ranked bar chart. Sorts
// rows by their numeric value and draws a labelled bar per row, scaled to the
// largest value — the canonical "top N" view. Generic + dependency-free (CSS
// bars), so any row array drives it.
...
const {
   ...
   * had the producer finished. The span between `valueKey` and this draws
   * hatched: observed, then unobserved. Without it a short bar reads as
   * "measured and small" when it often means "we stopped looking".
   */
  ceilingKey?: keyof T & string
  /**
   * Row key holding a truthy "this value is a floor" flag — the true count is
   * at or above what is drawn. The whole bar hatches and the value label
   * renders `≥ n`.
   */
...
<template>
  ...
  >
    ...
    >
      ...
      <div
        data-bar-track
        class="ui-bar-chart__track relative overflow-hidden rounded-sm bg-muted"
      ...
      >
        ...
        <div
          v-if="r.truncated"
          class="provisional-hatch absolute inset-y-0 left-0 rounded-sm ring-1 ring-current/35 ring-inset"
          :style="{ width: `${r.pct}%`, color }"
        />
        <div
          v-else
          class="absolute inset-y-0 left-0 w-full origin-left rounded-sm"
        ...
        <div
          ...
          class="provisional-hatch absolute inset-y-0 rounded-r-sm ring-1 ring-current/30 ring-inset"
          :style="{ left: `${r.pct}%`, width: `${r.provisionalPct}%`, color }"
        />
      </div>
      <span class="ui-bar-chart__value text-right font-mono text-highlighted">
        {{ fmtValue(r) }}
...
/* 14px floor: the source's 12px small rows read the same size here. */
...
/* The unobserved span: a diagonal hatch in the bar's own colour. */
.provisional-hatch {
