...
import { parseReportingDay } from '../utils/formatting.ts'
...
// Ported from the nuxtseo design-system, where it replaced three identical copies.
...
// the label never drifts with the viewer's timezone.
const weekdayDayFmt = new Intl.DateTimeFormat('en', { weekday: 'short', day: 'numeric', timeZone: 'UTC' })
...
const monthYearFmt = new Intl.DateTimeFormat('en', { month: 'short', year: '2-digit', timeZone: 'UTC' })
...
export function useChartTickPlan(opts: UseChartTickPlanOptions) {
  ...
  /** Label for one data index, or an empty string when it carries no tick. */
