...
describe('summariseBand', () => {
  ...
  it('counts agreement, extra Reviews, and skips of read Reviews per band', () => {
    expect(summariseBand(rows, 0.5)).toEqual({ band: 0.5, agreed: 3, reviewsAdded: 0, skipsAdded: 1, skipPrecision: 0.5 })
    expect(summariseBand(rows, 0.9)).toEqual({ band: 0.9, agreed: 4, reviewsAdded: 0, skipsAdded: 0, skipPrecision: 1 })
  })
  ...
  it('counts an over-confident skip as an added Review, never a lost one', () => {
    ...
    ], 0.7)).toEqual({ band: 0.7, agreed: 1, reviewsAdded: 1, skipsAdded: 0, skipPrecision: 1 })
  })
})
