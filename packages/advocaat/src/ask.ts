...
import type { ChoiceAnswer, ChoiceCriteria, ChoiceQuestion, Entry, JevOptions, Json, NoulQuestion, Question, RequestOptions, ScoreAnswer, ScoreCriteria, ScoreQuestion } from './api.ts'
import {
...
export async function ask<const Q extends AskQuestions>(
  ...
  if (inputs.length > 0) {
    ...
    if (Object.hasOwn(merged, 'input'))
      throw new Error('state already has "input"')
  ...
  const { answers } = await jev(options).systemOne(
